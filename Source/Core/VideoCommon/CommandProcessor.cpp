...
void SCPFifoStruct::Init()
{
  ...
  bFF_Breakpoint = 0;
  bFF_HiWatermark = 0;
  bFF_HiWatermarkInt = 0;
  bFF_LoWatermark = 0;
  bFF_LoWatermarkInt = 0;
...
static inline void WriteHigh(u32& reg, u16 highbits)
{
  reg = (reg & 0x0000FFFF) | (static_cast<u32>(highbits) << 16);
}
...
void RegisterMMIO(MMIO::Mapping* mmio, u32 base)
{
  ...
  mmio->Register(base | FIFO_RW_DISTANCE_HI,
                 MMIO::ComplexRead<u16>([](u32) {
                       Fifo::SyncGPUForRegisterAccess();
                       return fifo.CPReadWriteDistance >> 16;
                     }),
...
void GatherPipeBursted()
{
  ...
  ASSERT_MSG(COMMANDPROCESSOR,
             fifo.CPReadWriteDistance <= fifo.CPEnd - fifo.CPBase,
             "FIFO is overflowed by GatherPipe !\nCPU thread is too fast!");

  // check if we are in sync
  ASSERT_MSG(COMMANDPROCESSOR,
             fifo.CPWritePointer == ProcessorInterface::Fifo_CPUWritePointer,
             "FIFOs linked but out of sync");
  ASSERT_MSG(COMMANDPROCESSOR,
             fifo.CPBase == ProcessorInterface::Fifo_CPUBase,
             "FIFOs linked but out of sync");
  ASSERT_MSG(COMMANDPROCESSOR,
             fifo.CPEnd == ProcessorInterface::Fifo_CPUEnd,
             "FIFOs linked but out of sync");
}
...
{
  // breakpoint
  const bool breakpoint = fifo.bFF_Breakpoint;
  if (fifo.bFF_BPEnable != 0)
  {
...
void SetCpStatusRegister()
{
  ...
  m_CPStatusReg.ReadIdle = !fifo.CPReadWriteDistance ||
                           (fifo.CPReadPointer == fifo.CPWritePointer);
  m_CPStatusReg.CommandIdle = !fifo.CPReadWriteDistance || Fifo::AtBreakpoint() ||
...
void HandleUnknownOpcode(u8 cmd_byte, const u8* buffer, bool preprocess)
{
  ...
  GENERIC_LOG_FMT(
      ...
      fifo.bFF_LoWatermarkInt ? "true" : "false", PC, LR);
