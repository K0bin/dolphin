//
// Created by robin on 20/09/2023.
//
...
namespace Vulkan
{
    // TODO: Add better implementation using VK_NV_device_diagnostic_checkpoints and VK_AMD_buffer_marker

    static const u64 STAGING_BUFFER_SIZE = 4 << 20;

    VkDebug::VkDebug()
      : m_enabled(true)
    {
        m_readback_buffer = StagingBuffer::Create(STAGING_BUFFER_TYPE_READBACK, sizeof(u64), VK_BUFFER_USAGE_TRANSFER_DST_BIT);
    ...
    {
        if (m_buffers.size() != 0 && m_staging_buffer_offset < STAGING_BUFFER_SIZE - sizeof(u64)) [[likely]]
        {
            return;
        }
        ...
        std::unique_ptr<StagingBuffer> buffer = StagingBuffer::Create(STAGING_BUFFER_TYPE_UPLOAD, STAGING_BUFFER_SIZE, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
        buffer->Map();
    ...
    void VkDebug::EmitMarker(VkDebugCommand cmd, u64 aux0, u64 aux1, u64 aux2, u64 aux3)
    {
        ...
        *ptr = seq;
        ...
        VkDebugMarker marker = {seq, cmd, aux};
        m_markers.emplace_back(marker);

        VkMemoryBarrier memory_barrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT };
        vkCmdPipelineBarrier(g_command_buffer_mgr->GetCurrentCommandBuffer(), VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 1, &memory_barrier, 0, nullptr, 0, nullptr);
        VkBufferCopy copy = { m_staging_buffer_offset, 0, sizeof(u64) };
        vkCmdCopyBuffer(g_command_buffer_mgr->GetCurrentCommandBuffer(), buffer->GetBuffer(), m_readback_buffer->GetBuffer(), 1, &copy);
        m_readback_buffer->FlushGPUCache(g_command_buffer_mgr->GetCurrentCommandBuffer(), VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, sizeof(u64));
    ...
    void VkDebug::PrintFault()
    {
        ...
        ERROR_LOG_FMT(HOST_GPU, "Vulkan fault in seq {}, {}, aux0: {}, aux1: {}, aux2: {}, aux3: {}", fault->sequence_number, command_name, fault->aux[0], fault->aux[1], fault->aux[2], fault->aux[3]);
    }
}
